      ...
      <TableHead>
        <TableRow>
            ...
            <TableCell
              className={classes.tableHeading}
              key={'job_location'}
              sortDirection={orderBy === 'job_location' ? order : false}
            >
              <TableSortLabel
                active={orderBy === 'job_location'}
                direction={orderBy === 'job_location' ? order : 'asc'}
                onClick={createSortHandler('job_location')}
              >
                Job Location
            ...
            <TableCell
              className={classes.tableHeading}
              key={'approval'}
              sortDirection={orderBy === 'approval' ? order : false}
            >
              <TableSortLabel
                active={orderBy === 'approval'}
                direction={orderBy === 'approval' ? order : 'asc'}
                onClick={createSortHandler('approval')}
              >
               Approval
                {orderBy === 'approval' ? (
                  <Box component="span" sx={visuallyHidden}>
...
function Requirements () {
    ...
    React.useEffect(() => {
        setOpen(true)
          axios({
            method: 'get',
            url: 'http://localhost:8080/requirements',
          })
            .then(  (response) => {
    ...
    const  deleteUser = (candidateid) => {
        ...
        axios({
            method: 'get',
            url: 'http://localhost:8080/requirement/delete/'+candidateid,
        }).then((response) =>{
            ...
            <Container>
                ...
                <Grid container item xs={12} className={classes.Grid}>
                    ...
                    <Box className={classes.Box}>
                        ...
                        <Button component={Link} to="/requirements/add" variant="outlined" color="primary" className={classes.addCandidate}> + Add Requirement</Button>
                ...
                (<Table className={classes.table}>
                    {/* <TableHead>
                        <TableRow>
                            ...
                            <TableCell className={classes.tableHeading}>Department</TableCell>
                            ...
                            <TableCell className={classes.tableHeading}>Project Name</TableCell>
                            <TableCell className={classes.tableHeading}>Approval</TableCell>
                    ...
                    <TableBody>
                        ...
                        : filterdata).map((item, index) => {
                            return(
                                ...
                                <TableCell className={classes.tableRow}>{item.position}</TableCell>
                                <TableCell className={classes.tableRow}>{item.department}</TableCell>
                                <TableCell className={classes.tableRow}>{item.job_location}</TableCell>
                                <TableCell align="center">{item.experience_level}</TableCell>
                                <TableCell className={classes.tableRow}>{item.project_name}</TableCell>
                                <TableCell className={classes.tableRow}>{item.approval === '0' ? "No": "yes"}</TableCell>
                                <TableCell className={classes.tableRow}>
                                <Link  
                                  className={classes.editLinks} 
                                  to = {{
                                        pathname: "/requirement/update/",
                                        id: item._id
                                        }}
                                >
