...
const Form = () => {
  ...
  const handleSubmit = (event) => {
    ...
    // let cand = JSON.stringify(ls);
    // axios.post("http://localhost:8080/candidates/add", ls).then(res => {
    //         console.log(res.data);
    //     });
    axios({
      method: 'post',
      url: 'http://localhost:8080/requirements/add',
      data:formValues,
  ...
  return (
    ...
    <Box sx={{ width: '100%', padding:10 }}>
      ...
      <Grid item xs={6}>
        ...
        <Grid item xs={6}>
          <FormControl className={classes.formControl} required>
          <Typography variant="caption">Position Type</Typography>
            <Select
              name="position_type"
              variant="outlined"
